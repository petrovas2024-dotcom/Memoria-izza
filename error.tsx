...
const navItems = [
  ...
  { id: "ordenes" as View, label: "Órdenes", icon: ClipboardList },
...
export default function HomePage() {
  ...
  if (!authReady) return <div className="auth-loading"><div><strong>IZZA SMART</strong><span>Preparando acceso seguro…</span></div></div>;
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        ...
        <nav>
          ...
          {profile.role === "admin" && <><p className="nav-label">ADMINISTRACIÓN</p>
          {allowedNav.filter(item => ["tecnicos", "usuarios", "configuracion"].includes(item.id)).map(({ id, label, icon: Icon }) => <button key={id} className={view === id ? "active" : ""} onClick={() => navigate(id)}><Icon size={19}/><span>{label}</span>{view === id && <ChevronRight size={16}/>}</button>)}</>}
        </nav>
        <div className="connection-card"><span>{cloudStatus === "connected" ? "SUPABASE CONECTADO" : cloudStatus === "offline" ? "CONEXIÓN INTERRUMPIDA" : "CONECTANDO..."}</span><p>{cloudStatus === "connected" ? "Los datos se leen y guardan directamente en producción." : "No se guardarán cambios hasta recuperar la conexión."}</p><button onClick={() => notify(cloudStatus === "connected" ? "Conexión de producción activa" : "Reintentaremos la conexión automáticamente")}>{cloudStatus === "connected" ? "Producción activa" : "Ver estado"}</button></div>
        <div className="sidebar-foot"><p>Servicio bilingüe</p><small>Bilingual Service · Tijuana, B.C.</small></div>
      </aside>
      ...
      <main>
        <header className="topbar">
          ...
          <div className="top-actions">
            ...
            <button className="user-chip logout-chip" onClick={logout} title="Cerrar sesión"><div>{profile.full_name.split(" ").map(x => x[0]).slice(0,2).join("")}</div><span><strong>{profile.full_name}</strong><small>{roleLabel} · Cerrar sesión</small></span></button>
          </div>
        </header>

        <div className="workspace">
          <section className="page-heading">
            <div><p className="eyebrow">IZZA SERVICIOS DE MANTENIMIENTO</p><h1>{navItems.find(n => n.id === view)?.label}</h1><p>{view === "panel" ? "Resumen operativo de hoy · Tijuana, Baja California" : "Gestiona la información de forma rápida y segura."}</p></div>
...
function LoginScreen({ onAuthenticated }: { onAuthenticated: (profile: Profile, token: string) => void }) {
  ...
  const recover = async () => {
    ...
    setMessage(error ? "No fue posible enviar el correo de recuperación." : "Te enviamos un enlace para restablecer tu contraseña.");
  };
  ...
  return <main className="login-page">
    <section className="login-card auth-error-card" role="alert">
      ...
      <button className="primary-button" disabled={busy} onClick={async () => {
        ...
        await onLogout();
        setBusy(false);
      }}>{busy ? "Cerrando sesión…" : "Cerrar sesión y volver al acceso"}</button>
    </section>
  </main>;
}
...
function TechnicianWorkspace({ profile, orders, accessToken, setOrders, onLogout, notify, toast, materialQuoteOpen, openMaterials, closeMaterials, onMaterialSaved }: {
  ...
  return <main className="technician-page">
    <header className="tech-header"><div><Image src="/logo_izza.png" alt="IZZA" width={52} height={52} unoptimized/><span><strong>Mis órdenes</strong><small>{profile.full_name} · Técnico</small></span></div><button className="secondary-button" onClick={onLogout}>Cerrar sesión</button></header>
    <section className="tech-workspace">
      <div className="page-heading"><div><p className="eyebrow">TRABAJO ASIGNADO</p><h1>Órdenes de servicio</h1><p>Solo puedes consultar las órdenes asignadas a tu cuenta.</p></div></div>
      ...
      <div className="tech-order-list">{orders.map(order => <article className="card tech-order" key={order.folio}>
        ...
        <label>Observaciones<textarea value={order.observations || ""} onChange={e => setOrders(orders.map(item => item.folio === order.folio ? {...item, observations: e.target.value} : item))} placeholder="Describe el trabajo realizado, materiales o recomendaciones…"/></label>
        ...
        <div className="tech-save-actions"><button className="secondary-button" onClick={() => openMaterials(order)}><PackageSearch size={18}/>Cotizar materiales</button><button className="primary-button save-tech-order" disabled={saving === order.folio} onClick={() => saveOrder(order)}>{saving === order.folio ? "Guardando…" : "Guardar avance"}</button></div>
      </article>)}</div>
  ...
  return <div className="dashboard">
    ...
    <section className="dashboard-grid">
      <article className="card schedule-card">
        <div className="card-head"><div><p className="eyebrow">OPERACIÓN DE HOY</p><h2>Próximos servicios</h2></div><button className="text-button" onClick={() => navigate("agenda")}>Ver agenda <ArrowUpRight size={16}/></button></div>
        <div className="timeline">{todays.map((s, i) => <div className="timeline-item" key={s.id}><div className="time"><strong>{s.time}</strong><span>{s.end}</span></div><div className="line"><i className={i === 0 ? "current" : ""}/></div><div className="service-info"><div><strong>{s.client}</strong><StatusBadge status={s.status}/></div><p><Wrench size={14}/>{s.type} · {s.technician}</p><p><MapPin size={14}/>{s.address}</p></div><a href={`https://wa.me/52${s.phone}`} target="_blank" aria-label={`WhatsApp de ${s.client}`}><MessageCircle size={18}/></a></div>)}</div>
      </article>
      ...
      <article className="card alerts-card">
        <div className="card-head"><div><p className="eyebrow">ATENCIÓN</p><h2>Alertas importantes</h2></div><span className="count">{unansweredQuotes + (outstanding > 0 ? 1 : 0)}</span></div>
      ...
      <article className="card quick-card"><div className="card-head"><div><p className="eyebrow">ACCESOS RÁPIDOS</p><h2>¿Qué quieres hacer?</h2></div></div><div className="quick-grid">
...
function Agenda({ services, clients, technicians, mode, setMode, notify }: { services: Service[]; clients: Client[]; technicians: TechnicianCard[]; mode: string; setMode: (m:string)=>void; notify:(m:string)=>void }) {
  ...
  const periodLabel = mode === "Día" ? label(anchor,{weekday:"long",day:"numeric",month:"long",year:"numeric"}) : mode === "Semana" ? `${label(weekStart,{day:"numeric",month:"short"})} – ${label(addDays(weekStart,6),{day:"numeric",month:"short",year:"numeric"})}` : label(anchor,{month:"long",year:"numeric"});
...
function Orders({ orders, clients, technicians, notify, openMaterials, onAssign }: { orders: WorkOrder[]; clients: Client[]; technicians: TechnicianCard[]; notify:(m:string)=>void; openMaterials:(order:WorkOrder)=>void; onAssign:(orderId:number, technician:string)=>void }) {
  return <section className="card module-card"><div className="module-head"><div><h2>Órdenes de servicio</h2><p>Seguimiento técnico, evidencia y firmas.</p></div><button className="primary-button" onClick={()=>notify("Selecciona una cotización aprobada para convertirla")}><Plus size={18}/>Nueva orden</button></div>
...
function MaterialQuoteEditor({ quote, role, accessToken, onClose, onSaved, onFormalQuote, notify }: { quote:MaterialQuote; role:AppRole; accessToken:string; onClose:()=>void; onSaved:(quote:MaterialQuote)=>void; onFormalQuote?:(quote:Quote)=>void; notify:(message:string)=>void }) {
  ...
  return <div className="material-editor-backdrop"><section className="material-editor" role="dialog" aria-modal="true"><header><div><p className="eyebrow">ORDEN {draft.work_order_folio}</p><h2>Cotizador de materiales</h2><span>{draft.client} · {draft.technician}</span><small className={`material-save-state ${saveState}`}>{saveState==="saving"?"Guardando…":saveState==="saved"?"Guardado automáticamente":saveState==="error"?"Error al guardar":"Los cambios se guardan automáticamente"}</small></div><button onClick={onClose} aria-label="Cerrar"><X/></button></header>
...
function Technicians({ techs, onSave, onTechniciansChanged, notify }: {
  ...
  const openNewTechnician = () => {
    setCreationMessage({ type: "diagnostic", text: "Botón recibido correctamente. Abriendo formulario…" });
  ...
  return <section className="card module-card">
    ...
    {editing && <div className="modal-backdrop" onMouseDown={() => setEditing(null)}><div className="modal technician-modal" onMouseDown={e => e.stopPropagation()}>
      <div className="modal-head"><div><p className="eyebrow">EQUIPO TÉCNICO</p><h2>Editar técnico</h2></div><button onClick={() => setEditing(null)} aria-label="Cerrar"><X/></button></div>
      <div className="form-grid">
        ...
        <label className="wide">Especialidad<input value={editing.specialty} onChange={e => setEditing({...editing, specialty:e.target.value})} placeholder="Ej. Plomería · Boilers"/></label>
    ...
    {creating && <div className="modal-backdrop user-modal-backdrop" onMouseDown={() => !creatingUser && setCreating(null)}>
      <form id="new-technician-dialog" role="dialog" aria-modal="true" aria-labelledby="new-technician-title" className="modal user-modal" onSubmit={createTechnician} onMouseDown={event => event.stopPropagation()}>
        <div className="modal-head"><div><p className="eyebrow">NUEVO ACCESO TÉCNICO</p><h2 id="new-technician-title">Crear técnico</h2></div><button type="button" disabled={creatingUser} onClick={() => setCreating(null)} aria-label="Cerrar formulario"><X/></button></div>
        <div className="form-grid">
          ...
          <label className="wide">Especialidad<input value={creating.specialty} onChange={event => setCreating({ ...creating, specialty: event.target.value })} placeholder="Ej. Plomería · Electricidad · Boilers"/></label>
          ...
          <div className="modal-actions wide"><button type="button" className="secondary-button" disabled={creatingUser} onClick={() => setCreating(null)}>Cancelar</button><button type="submit" className="primary-button" disabled={creatingUser}>{creatingUser ? "Creando técnico…" : "Crear técnico"}</button></div>
        </div>
      </form>
    </div>}
  </section>;
...
function UserAdministration({ notify, onTechniciansChanged }: { notify: (message: string) => void; onTechniciansChanged: () => Promise<void> }) {
  ...
  return <section className="module-card user-admin">
    <div className="module-head"><div><p className="eyebrow">ADMINISTRACIÓN DEL SISTEMA</p><h2>Usuarios y accesos</h2><p>Crea cuentas, asigna roles y administra el acceso sin entrar a Supabase.</p></div><button type="button" className="primary-button" aria-haspopup="dialog" aria-controls="new-user-dialog" onClick={openNewUser}><UserPlus/>Nuevo usuario</button></div>
    ...
    {loading ? <div className="user-empty">Consultando accesos…</div> : users.length === 0 ? <div className="user-empty">No hay usuarios registrados.</div> :
      <div className="user-table"><div className="user-row user-header"><span>Usuario</span><span>Rol</span><span>Estado</span><span>Último acceso</span><span>Acciones</span></div>
    ...
    {editing && <div className="modal-backdrop user-modal-backdrop" onMouseDown={() => !saving && setEditing(null)}><form id="new-user-dialog" role="dialog" aria-modal="true" aria-labelledby="user-dialog-title" className="modal user-modal" onSubmit={event => { event.preventDefault(); void save(); }} onMouseDown={event => event.stopPropagation()}>
      ...
      <div className="form-grid">
        ...
        <div className="modal-actions wide"><button type="button" className="secondary-button" disabled={saving} onClick={() => setEditing(null)}>Cancelar</button><button type="submit" className="primary-button" disabled={saving}>{saving ? "Guardando…" : editing.id ? "Guardar cambios" : "Crear usuario"}</button></div>
      </div>
    </form></div>}
  </section>;
}
