...
async function getSupabaseAccessToken(forceRefresh = false): Promise<string> {
  if (!supabase) throw new Error("La conexiÃ³n segura de Supabase no estÃ¡ disponible.");
  let result = forceRefresh
    ? await withTimeout(supabase.auth.refreshSession(), "Supabase tardÃ³ demasiado en renovar la sesiÃ³n.")
    : await withTimeout(supabase.auth.getSession(), "Supabase tardÃ³ demasiado en recuperar la sesiÃ³n.");
  const expiresAt = result.data.session?.expires_at || 0;
  if (!forceRefresh && result.data.session && expiresAt * 1000 <= Date.now() + 60_000) {
    result = await withTimeout(supabase.auth.refreshSession(), "Supabase tardÃ³ demasiado en renovar la sesiÃ³n.");
  }
  if (result.error || !result.data.session?.access_token) {
    throw new Error("Tu sesiÃ³n terminÃ³. Cierra sesiÃ³n e ingresa nuevamente.");
  }
...
const serviceSchema = z.object({
  ...
  technician: z.string().min(2, "Asigna un tÃ©cnico"),
...
const clientSchema = z.object({
  ...
  phone: z.string().min(10, "Escribe un telÃ©fono vÃ¡lido"),
  email: z.string().email("Correo no vÃ¡lido"),
  address: z.string().min(5, "Escribe la direcciÃ³n"),
});
...
const navItems = [
  ...
  { id: "tecnicos" as View, label: "TÃ©cnicos", icon: Wrench },
  { id: "usuarios" as View, label: "Usuarios y accesos", icon: ShieldCheck },
  { id: "configuracion" as View, label: "ConfiguraciÃ³n", icon: Settings },
];
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    setAuthError("");
    if (!supabase) {
      setAuthError("La conexiÃ³n segura de IZZA Smart no estÃ¡ disponible. La configuraciÃ³n de Supabase no se cargÃ³ correctamente.");
      setAuthReady(true);
      return;
    }
    withTimeout(
      supabase.auth.getSession(),
      "Supabase tardÃ³ demasiado en recuperar la sesiÃ³n."
      ...
      if (!result.profile?.role || !result.profile?.active) throw new Error("Tu perfil de IZZA Smart no estÃ¡ activo o no tiene un rol vÃ¡lido.");
      if (!active) return;
  ...
  const loadTechnicians = async () => {
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || "No se pudo consultar el catÃ¡logo de tÃ©cnicos");
      setTechnicians(result.technicians || []);
    } catch {
      setTechnicians([]);
      notify("No se pudo actualizar el catÃ¡logo de tÃ©cnicos");
    }
  };
  ...
  const updateTechnician = (updated: TechnicianCard, previousName: string) => {
    ...
    notify("TÃ©cnico actualizado; guardando en Supabase");
  };
  const assignOrder = (orderId: number, technician: string) => {
    ...
    notify(technician === "Sin asignar" ? "Orden marcada sin tÃ©cnico" : `Orden asignada a ${technician}`);
  };
  ...
  const saveQuote = async () => {
    ...
    if (quoteManualTotal && (quoteTotalInput === "" || !Number.isFinite(parsedQuoteTotal) || parsedQuoteTotal < 0)) {
      notify("Escribe un total vÃ¡lido"); return;
    }
    ...
    const savedQuote: Quote = {
      ...
      subtotal: quoteSubtotal, tax: quoteIva, discount: quoteDiscount, total: quoteTotal, validity: existing?.validity || "15 dÃ­as",
    ...
    try {
      ...
      notify(existing ? "CotizaciÃ³n actualizada en Supabase" : "CotizaciÃ³n guardada en Supabase");
    } catch {
      setCloudStatus("offline");
      notify("No se pudo guardar. Revisa la conexiÃ³n e intenta de nuevo");
    } finally {
  ...
  const runQuoteWorkflow = async (quote: Quote, action: "approve" | "convert" | "reject") => {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || "No se pudo completar la acciÃ³n");
      await refreshOperationalData();
      if (action !== "reject") setView("ordenes");
      if (action === "reject") notify(`CotizaciÃ³n ${quote.folio} rechazada y guardada en Supabase`);
      else if (action === "approve") notify(result.created
        ? `CotizaciÃ³n ${quote.folio} aprobada y orden creada correctamente`
        : `CotizaciÃ³n ${quote.folio} aprobada; su orden ya existÃ­a y no se duplicÃ³`);
      else notify(result.created ? "Orden de servicio creada correctamente" : "Esta cotizaciÃ³n ya tenÃ­a una orden; no se duplicÃ³");
    } catch (error) {
      notify(error instanceof Error ? error.message : "No se pudo completar la acciÃ³n");
  ...
  const roleLabel = profile.role === "admin" ? "Administrador" : "RecepciÃ³n";
  const logout = async () => { if (supabase) await supabase.auth.signOut(); setProfile(null); setAccessToken(""); setAuthError(""); };

  return (
    <div className="app-shell">
      <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
        <button className="close-menu" onClick={() => setMobileOpen(false)} aria-label="Cerrar menÃº"><X /></button>
        <div className="brand">
          <Image src="/logo_izza.png" alt="IZZA Servicios de Mantenimiento" width={54} height={54} priority unoptimized />
          <div><strong>IZZA SMART</strong><span>AdministraciÃ³n integral</span></div>
        </div>
        <nav>
        ...
        <div className="connection-card"><span>{cloudStatus === "connected" ? "SUPABASE CONECTADO" : cloudStatus === "offline" ? "CONEXIÃ“N INTERRUMPIDA" : "CONECTANDO..."}</span><p>{cloudStatus === "connected" ? "Los datos se leen y guardan directamente en producciÃ³n." : "No se guardarÃ¡n cambios hasta recuperar la conexiÃ³n."}</p><button onClick={() => notify(cloudStatus === "connected" ? "ConexiÃ³n de producciÃ³n activa" : "Reintentaremos la conexiÃ³n automÃ¡ticamente")}>{cloudStatus === "connected" ? "ProducciÃ³n activa" : "Ver estado"}</button></div>
      ...
      <main>
        <header className="topbar">
          <button className="menu-button" onClick={() => setMobileOpen(true)} aria-label="Abrir menÃº"><Menu /></button>
          <div className="global-search"><Search size={19}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar cliente, folio o telÃ©fono..." /></div>
          <div className="top-actions">
            ...
            <button className="user-chip logout-chip" onClick={logout} title="Cerrar sesiÃ³n"><div>{profile.full_name.split(" ").map(x => x[0]).slice(0,2).join("")}</div><span><strong>{profile.full_name}</strong><small>{roleLabel} Â· Cerrar sesiÃ³n</small></span></button>
          </div>
        </header>

        <div className="workspace">
          <section className="page-heading">
            <div><p className="eyebrow">IZZA SERVICIOS DE MANTENIMIENTO</p><h1>{navItems.find(n => n.id === view)?.label}</h1><p>{view === "panel" ? "Resumen operativo de hoy Â· Tijuana, Baja California" : "Gestiona la informaciÃ³n de forma rÃ¡pida y segura."}</p></div>
            <div className="heading-actions">
              <button className="secondary-button" onClick={openNewQuote}><FileText size={18}/>Nueva cotizaciÃ³n</button>
      ...
      {modal && <div className="modal-backdrop" onMouseDown={() => setModal(null)}><div className="modal" onMouseDown={e => e.stopPropagation()}>
        <div className="modal-head"><div><p className="eyebrow">{modal === "quote" && editingQuoteId ? "EDITAR REGISTRO" : "NUEVO REGISTRO"}</p><h2>{modal === "service" ? "Programar servicio" : modal === "client" ? "Registrar cliente" : modal === "quote" ? editingQuoteId ? "Editar cotizaciÃ³n" : "Nueva cotizaciÃ³n" : "Registrar pago"}</h2></div><button onClick={() => setModal(null)} aria-label="Cerrar"><X/></button></div>
        {modal === "service" && <form onSubmit={serviceForm.handleSubmit(createService)} className="form-grid">
          ...
          <label>Tipo de servicio<select {...serviceForm.register("type")}><option value="">Selecciona...</option>{["PlomerÃ­a","Electricidad","Boilers","Bombas de agua","ImpermeabilizaciÃ³n","RefrigeraciÃ³n","Minisplit","Mantenimiento general"].map(x => <option key={x}>{x}</option>)}</select><em>{serviceForm.formState.errors.type?.message}</em></label>
          <label>TÃ©cnico<select {...serviceForm.register("technician")}><option value="">Selecciona...</option>{technicians.filter(technician => technician.active).map(technician => <option key={technician.id} value={technician.name}>{technician.name}</option>)}</select><em>{serviceForm.formState.errors.technician?.message}</em></label>
        ...
        {modal === "client" && <form onSubmit={clientForm.handleSubmit(createClient)} className="form-grid">
          ...
          <label>TelÃ©fono / WhatsApp<input {...clientForm.register("phone")} placeholder="664 000 0000"/><em>{clientForm.formState.errors.phone?.message}</em></label>
          ...
          <label className="wide">DirecciÃ³n<input {...clientForm.register("address")} placeholder="Calle, nÃºmero, colonia, ciudad"/><em>{clientForm.formState.errors.address?.message}</em></label>
        ...
        {modal === "quote" && <div className="quote-form">
          <div className="form-grid"><label>Cliente<select value={quoteClient} onChange={e => setQuoteClient(e.target.value)} disabled={clients.length === 0}>{clients.length === 0 ? <option>Primero registra un cliente</option> : clients.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}</select></label><label>Servicio<select value={quoteType} onChange={e => setQuoteType(e.target.value)}><option>Servicio general</option><option>Minisplit</option><option>PlomerÃ­a</option><option>Electricidad</option><option>ImpermeabilizaciÃ³n</option><option>Boilers</option></select></label></div>
          ...
          <div className="quote-total"><span>Subtotal <b>{money.format(quoteSubtotal)}</b></span><span>IVA <b>{money.format(quoteIva)}</b></span><span>Descuento <b>-{money.format(quoteDiscount)}</b></span><strong><label htmlFor="quote-total-input">TOTAL</label><input id="quote-total-input" aria-label="Total de la cotizaciÃ³n" inputMode="decimal" type="text" value={quoteManualTotal ? quoteTotalInput : calculatedQuoteTotal.toFixed(2)} onFocus={e => { if (!quoteManualTotal) { setQuoteTotalInput(calculatedQuoteTotal.toFixed(2)); setQuoteManualTotal(true); requestAnimationFrame(() => e.currentTarget.select()); } }} onChange={e => { const value = e.target.value; if (/^\d*(?:[.,]\d{0,2})?$/.test(value)) setQuoteTotalInput(value); }}/></strong></div>
          <div className="quote-total-mode"><span>{quoteManualTotal ? "Total capturado manualmente" : "Total calculado automÃ¡ticamente"}</span>{quoteManualTotal && <button type="button" className="text-button" onClick={() => { setQuoteManualTotal(false); setQuoteTotalInput(""); }}>Usar cÃ¡lculo automÃ¡tico</button>}</div>
          <div className="modal-actions"><button className="secondary-button" onClick={() => setModal(null)}>Cancelar</button><button className="primary-button" disabled={clients.length === 0 || savingQuote} onClick={saveQuote}>{savingQuote ? "Guardando..." : editingQuoteId ? "Actualizar cotizaciÃ³n" : "Guardar borrador"}</button></div>
        </div>}
        {modal === "payment" && <div className="form-grid">
          ...
          <label>Importe<input type="number" defaultValue="1000"/></label><label>MÃ©todo<select><option>Transferencia</option><option>Efectivo</option><option>Tarjeta</option><option>DepÃ³sito</option></select></label>
...
function LoginScreen({ onAuthenticated }: { onAuthenticated: (profile: Profile, token: string) => void }) {
  ...
  const login = async (event: FormEvent) => {
    ...
    try {
      if (!supabase) throw new Error("La conexiÃ³n segura no estÃ¡ disponible.");
      const { data, error } = await supabase.auth.signInWithPassword({ email, password });
      if (error || !data.session) { setMessage("Correo o contraseÃ±a incorrectos."); return; }
      const response = await fetch("/api/auth/profile", {
      ...
      if (!response.ok || !result.profile) {
        setMessage(result.error || "Tu usuario no tiene un perfil vÃ¡lido.");
        setProfileFailure(true);
        return;
      }
      onAuthenticated(result.profile, data.session.access_token);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "No fue posible iniciar sesiÃ³n.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return <main className="login-page">
    <section className="login-card">
      ...
      <p>Ingresa con el usuario asignado por administraciÃ³n.</p>
      <form onSubmit={login}>
        <label>Correo electrÃ³nico<input type="email" autoComplete="username" value={email} onChange={e => setEmail(e.target.value)} required /></label>
        <label>ContraseÃ±a<input type="password" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} required /></label>
        {message && <div className="auth-message">{message}</div>}
        <button className="primary-button" disabled={busy}>{busy ? "Ingresandoâ€¦" : "Iniciar sesiÃ³n"}</button>
        {profileFailure && <button type="button" className="secondary-button" onClick={async () => {
          if (supabase) await supabase.auth.signOut().catch(() => undefined);
          setProfileFailure(false); setPassword(""); setMessage("SesiÃ³n cerrada. Puedes volver a intentarlo.");
        }}>Cerrar sesiÃ³n</button>}
        <button type="button" className="text-button recover-button" onClick={recover}>Â¿Olvidaste tu contraseÃ±a?</button>
      </form>
    </section>
  </main>;
}
...
function AuthErrorScreen({ message, onRetry, onLogout }: { message: string; onRetry: () => void; onLogout: () => Promise<void> }) {
  ...
  return <main className="login-page">
    <section className="login-card auth-error-card" role="alert">
      ...
      <p>{message}</p>
      <p className="auth-help">Tus datos no se modificaron. Puedes reintentar la conexiÃ³n o cerrar la sesiÃ³n para volver al acceso.</p>
      ...
      <button className="primary-button" disabled={busy} onClick={async () => {
        ...
        await onLogout();
        setBusy(false);
      }}>{busy ? "Cerrando sesiÃ³nâ€¦" : "Cerrar sesiÃ³n y volver al acceso"}</button>
    </section>
  </main>;
}
...
function TechnicianWorkspace({ profile, orders, accessToken, setOrders, onLogout, notify, toast, materialQuoteOpen, openMaterials, closeMaterials, onMaterialSaved }: {
  ...
  return <main className="technician-page">
    <header className="tech-header"><div><Image src="/logo_izza.png" alt="IZZA" width={52} height={52} unoptimized/><span><strong>Mis Ã³rdenes</strong><small>{profile.full_name} Â· TÃ©cnico</small></span></div><button className="secondary-button" onClick={onLogout}>Cerrar sesiÃ³n</button></header>
    <section className="tech-workspace">
      <div className="page-heading"><div><p className="eyebrow">TRABAJO ASIGNADO</p><h1>Ã“rdenes de servicio</h1><p>Solo puedes consultar las Ã³rdenes asignadas a tu cuenta.</p></div></div>
      {!orders.length && <div className="card empty"><ClipboardList/><strong>No tienes Ã³rdenes asignadas</strong><p>Cuando recepciÃ³n te asigne un servicio aparecerÃ¡ aquÃ­.</p></div>}
      <div className="tech-order-list">{orders.map(order => <article className="card tech-order" key={order.folio}>
        ...
        <div className="tech-contact-actions">
          <a className="secondary-button" target="_blank" href={`https://maps.google.com/?q=${encodeURIComponent(order.address || "")}`}><MapPin size={17}/>CÃ³mo llegar</a>
...
function Dashboard({ services, quotes, payments, navigate, setModal, openServiceForm }: { services: Service[]; quotes: Quote[]; payments: Payment[]; navigate: (v: View) => void; setModal: (v: "service" | "quote") => void; openServiceForm: () => void }) {
  ...
  return <div className="dashboard">
    <section className="metrics-grid">
      ...
      <MetricCard label="Pendientes" value={String(services.filter(s => s.status.includes("Pendiente")).length)} note="Requieren confirmaciÃ³n" icon={Clock3} tone="gold"/>
    ...
    <section className="dashboard-grid">
      <article className="card schedule-card">
        <div className="card-head"><div><p className="eyebrow">OPERACIÃ“N DE HOY</p><h2>PrÃ³ximos servicios</h2></div><button className="text-button" onClick={() => navigate("agenda")}>Ver agenda <ArrowUpRight size={16}/></button></div>
      ...
      <article className="card quick-card"><div className="card-head"><div><p className="eyebrow">ACCESOS RÃPIDOS</p><h2>Â¿QuÃ© quieres hacer?</h2></div></div><div className="quick-grid">
        <button onClick={openServiceForm}><span><Plus/></span><strong>Nuevo servicio</strong><small>Agenda una visita</small></button>
        <button onClick={() => setModal("quote")}><span><FileText/></span><strong>Nueva cotizaciÃ³n</strong><small>Calcula y comparte</small></button>
        <button onClick={() => navigate("clientes")}><span><Users/></span><strong>Buscar cliente</strong><small>Consulta historial</small></button>
        <button onClick={() => navigate("ordenes")}><span><Camera/></span><strong>Subir evidencia</strong><small>Antes y despuÃ©s</small></button>
      </div></article>
    </section>
  </div>;
}
...
function Agenda({ services, clients, technicians, mode, setMode, notify }: { services: Service[]; clients: Client[]; technicians: TechnicianCard[]; mode: string; setMode: (m:string)=>void; notify:(m:string)=>void }) {
  ...
  const visibleDays = mode === "DÃ­a" ? [anchor] : Array.from({length:7},(_,index)=>addDays(weekStart,index));
  ...
  const periodLabel = mode === "DÃ­a" ? label(anchor,{weekday:"long",day:"numeric",month:"long",year:"numeric"}) : mode === "Semana" ? `${label(weekStart,{day:"numeric",month:"short"})} â€“ ${label(addDays(weekStart,6),{day:"numeric",month:"short",year:"numeric"})}` : label(anchor,{month:"long",year:"numeric"});
  return <section className="card module-card">
    <div className="toolbar"><div className="segmented">{["DÃ­a","Semana","Mes","Lista"].map(x => <button type="button" className={mode===x?"active":""} onClick={()=>setMode(x)} key={x}>{x}</button>)}</div><div className="filters"><select aria-label="Filtrar por tÃ©cnico" value={technicianFilter} onChange={event=>setTechnicianFilter(event.target.value)}><option value="">Todos los tÃ©cnicos</option>{technicians.filter(technician => technician.active).map(technician => <option key={technician.id} value={technician.name}>{technician.name}</option>)}</select><select aria-label="Filtrar por estado" value={statusFilter} onChange={event=>setStatusFilter(event.target.value)}><option value="">Todos los estados</option>{["Pendiente de confirmar","Confirmado","En camino","En proceso","Terminado","Reprogramado"].map(status=><option key={status}>{status}</option>)}</select></div></div>
...
function Quotes({ quotes, notify, openNew, openEdit, onApprove, onReject, onConvert, accessToken, busy }: { quotes: Quote[]; notify:(m:string)=>void; openNew:()=>void; openEdit:(quote:Quote)=>void; onApprove:(quote:Quote)=>void; onReject:(quote:Quote)=>void; onConvert:(quote:Quote)=>void; accessToken:string; busy:string }) {
  return <section className="card module-card"><div className="module-head"><div><h2>Cotizaciones recientes</h2><p>Calcula, aprueba y convierte en servicio.</p></div><div className="module-actions"><a className="secondary-button" href="/Cotizacion_IZZA_Premium_Automatica.docm" download><Download size={18}/>Plantilla automÃ¡tica</a><button className="primary-button" onClick={openNew}><Plus size={18}/>Nueva cotizaciÃ³n</button></div></div>
    <div className="quote-template-banner"><div><FileText/><span><strong>CotizaciÃ³n premium IZZA con VBA</strong><small>Abre en Word, habilita el contenido y captura cantidad, precio, IVA, descuento o anticipo: todos los totales se recalculan al salir del campo.</small></span></div><a href="/Cotizacion_IZZA_Premium_Automatica.docm" download>Descargar versiÃ³n automÃ¡tica <Download size={16}/></a></div>
    <div className="data-table"><div className="table-row quote-cols table-header"><span>Folio / fecha</span><span>Cliente</span><span>Servicio</span><span>Total</span><span>Estado</span><span>Acciones</span></div>{quotes.map(q=>{ const whatsapp=encodeURIComponent(`CotizaciÃ³n ${q.folio}\nCliente: ${q.client}\nServicio: ${q.type}\nTotal: ${money.format(q.total)}`); return <div className="table-row quote-cols" key={q.id}><span><strong>{q.folio}</strong><small>{q.date} Â· Vigencia {q.validity}</small></span><span>{q.client}</span><span>{q.type}</span><span><strong>{money.format(q.total)}</strong></span><span><StatusBadge status={q.status}/></span><span className="quote-workflow-actions"><div className="row-actions"><a href={`/api/quotes/pdf?folio=${encodeURIComponent(q.folio)}&token=${encodeURIComponent(accessToken)}`} target="_blank" rel="noreferrer" title="Descargar PDF" onClick={()=>notify("Generando PDF de la cotizaciÃ³n")}><Download size={17}/></a><a href={`https://wa.me/?text=${whatsapp}`} target="_blank" rel="noreferrer" title="Compartir por WhatsApp"><MessageCircle size={17}/></a><button onClick={()=>openEdit(q)} title="Editar cotizaciÃ³n"><MoreHorizontal size={17}/></button></div>{q.status==="Borrador"&&<><button className="reject-button" disabled={!!busy} onClick={()=>onReject(q)}>{busy===`${q.folio}:reject`?"Rechazandoâ€¦":"Rechazar"}</button><button className="approve-quote-button" disabled={!!busy} onClick={()=>onApprove(q)}><CheckCircle2/> {busy===`${q.folio}:approve`?"Aprobando y creandoâ€¦":"Aprobar y crear orden"}</button></>}{q.status==="Aprobada"&&<button className="convert-order-button" disabled={!!busy} onClick={()=>onConvert(q)}><ClipboardList/> {busy===`${q.folio}:convert`?"Creandoâ€¦":"Crear orden"}</button>}</span></div>})}</div>
  </section>;
}
...
function Orders({ orders, clients, technicians, notify, openMaterials, onAssign }: { orders: WorkOrder[]; clients: Client[]; technicians: TechnicianCard[]; notify:(m:string)=>void; openMaterials:(order:WorkOrder)=>void; onAssign:(orderId:number, technician:string)=>void }) {
  return <section className="card module-card"><div className="module-head"><div><h2>Ã“rdenes de servicio</h2><p>Seguimiento tÃ©cnico, evidencia y firmas.</p></div><button className="primary-button" onClick={()=>notify("Selecciona una cotizaciÃ³n aprobada para convertirla")}><Plus size={18}/>Nueva orden</button></div>
    {!orders.length && <div className="empty-materials"><ClipboardList/><strong>AÃºn no hay Ã³rdenes de servicio</strong><span>Al aprobar una cotizaciÃ³n, su orden aparecerÃ¡ aquÃ­ automÃ¡ticamente.</span></div>}
    <div className="order-grid">{orders.map(o=><article className="order-card" key={o.id}><div className="order-head"><span>{o.folio}</span><StatusBadge status={o.status}/></div><h3>{o.service}</h3><p><UserRound/> {o.client}</p><label className="order-assignment"><span><Wrench/>TÃ©cnico asignado</span><select value={o.technician || "Sin asignar"} onChange={event => onAssign(o.id, event.target.value)}><option value="Sin asignar">Sin asignar</option>{technicians.filter(technician => technician.active).map(technician => <option key={technician.id} value={technician.name}>{technician.name}</option>)}</select></label><PropertyAgendaSummary clientId={clients.find(client => client.name === o.client)?.id || 0}/><div className="order-meta"><span><CalendarDays/>{o.date}</span><span><Camera/>{o.evidence} fotos</span><strong>{money.format(o.total)}</strong></div><div className="order-actions"><button onClick={()=>notify("Orden abierta")}>Abrir orden</button><button onClick={()=>openMaterials(o)}><PackageSearch/> Cotizar materiales</button></div></article>)}</div>
  </section>;
}
...
function MaterialsModule({ quotes, orders, openQuote }: { quotes: MaterialQuote[]; orders: WorkOrder[]; openQuote:(order?:WorkOrder)=>void }) {
  return <section className="card module-card"><div className="module-head"><div><h2>Cotizador de materiales</h2><p>Compara proveedores y prepara el precio para el cliente.</p></div><button className="primary-button" onClick={()=>openQuote(orders[0])}><Plus size={18}/>Nueva cotizaciÃ³n</button></div>
...
function MaterialQuoteEditor({ quote, role, accessToken, onClose, onSaved, onFormalQuote, notify }: { quote:MaterialQuote; role:AppRole; accessToken:string; onClose:()=>void; onSaved:(quote:MaterialQuote)=>void; onFormalQuote?:(quote:Quote)=>void; notify:(message:string)=>void }) {
  ...
  const save = async (status=draft.status, quiet=false) => {
    ...
    try {
      ...
      if(!quiet) notify(status === "approved" ? "CotizaciÃ³n aprobada y guardada en Supabase" : status === "rejected" ? "CotizaciÃ³n rechazada y guardada en Supabase" : "CotizaciÃ³n de materiales guardada en Supabase");
  ...
  const convertToFormalQuote = async () => {
    ...
    try {
      ...
      const result=await response.json(); if(!response.ok) throw new Error(result.error);
      onFormalQuote?.(result.quote); notify("Partidas convertidas en cotizaciÃ³n formal");
    } catch(error) { notify(error instanceof Error?error.message:"No se pudo convertir la cotizaciÃ³n"); } finally { setSaving(false); }
  };
  const whatsappText = encodeURIComponent(`CotizaciÃ³n ${draft.folio||""}\nCliente: ${draft.client}\nMateriales: ${money.format(clientMaterials)}\nMano de obra: ${money.format(draft.labor)}\nTotal: ${money.format(generalTotal)}`);
  return <div className="material-editor-backdrop"><section className="material-editor" role="dialog" aria-modal="true"><header><div><p className="eyebrow">ORDEN {draft.work_order_folio}</p><h2>Cotizador de materiales</h2><span>{draft.client} Â· {draft.technician}</span><small className={`material-save-state ${saveState}`}>{saveState==="saving"?"Guardandoâ€¦":saveState==="saved"?"Guardado automÃ¡ticamente":saveState==="error"?"Error al guardar":"Los cambios se guardan automÃ¡ticamente"}</small></div><button onClick={onClose} aria-label="Cerrar"><X/></button></header>
...
function Payments({ payments, openNew }: { payments: Payment[]; openNew:()=>void }) {
  return <section className="card module-card"><div className="module-head"><div><h2>Movimientos registrados</h2><p>Total del periodo: {money.format(payments.reduce((a,p)=>a+p.amount,0))}</p></div><button className="primary-button" onClick={openNew}><Plus size={18}/>Registrar pago</button></div>
    <div className="data-table"><div className="table-row payment-cols table-header"><span>Recibo</span><span>Cliente</span><span>Fecha</span><span>Tipo</span><span>MÃ©todo</span><span>Importe</span></div>{payments.map(p=><div className="table-row payment-cols" key={p.id}><span><strong>{p.folio}</strong></span><span>{p.client}</span><span>{p.date}</span><span><StatusBadge status={p.type}/></span><span>{p.method}</span><span><strong>{money.format(p.amount)}</strong></span></div>)}</div>
  </section>;
}
...
function Technicians({ techs, onSave, onTechniciansChanged, notify }: {
  ...
  const openNewTechnician = () => {
    setCreationMessage({ type: "diagnostic", text: "BotÃ³n recibido correctamente. Abriendo formularioâ€¦" });
  ...
  const createTechnician = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (creating.full_name.trim().length < 3) return setCreationMessage({ type: "error", text: "Escribe el nombre completo del tÃ©cnico." });
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(creating.email.trim())) return setCreationMessage({ type: "error", text: "Escribe un correo electrÃ³nico vÃ¡lido." });
    if (creating.phone.trim() && creating.phone.replace(/\D/g, "").length < 10) return setCreationMessage({ type: "error", text: "El telÃ©fono debe tener al menos 10 dÃ­gitos." });
    if (creating.password.length < 8) return setCreationMessage({ type: "error", text: "La contraseÃ±a temporal debe tener al menos 8 caracteres." });
    if (creating.password !== creating.confirm_password) return setCreationMessage({ type: "error", text: "Las contraseÃ±as no coinciden." });
    ...
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.error || `Error ${response.status}: no se pudo crear el tÃ©cnico`);
      await onTechniciansChanged();
      window.dispatchEvent(new Event("izza:technicians-changed"));
      setCreating(null);
      setCreationMessage({ type: "success", text: `${creating.full_name.trim()} fue creado y ya estÃ¡ disponible en Usuarios, TÃ©cnicos y Programar servicio.` });
      notify("TÃ©cnico creado correctamente");
    } catch (failure) {
      const exactError = failure instanceof Error ? failure.message : "No se pudo crear el tÃ©cnico";
  ...
  return <section className="card module-card">
    <div className="module-head"><div><h2>Equipo tÃ©cnico</h2><p>Edita nombres, telÃ©fonos, especialidades y disponibilidad.</p></div><button type="button" className="primary-button" aria-haspopup="dialog" aria-controls="new-technician-dialog" onClick={openNewTechnician}><Plus size={18}/>Nuevo tÃ©cnico</button></div>
    ...
    <div className="tech-grid">{techs.map(t=><article className="tech-card" key={t.id}>
      ...
      <button className="edit-tech-button" onClick={() => { setPreviousName(t.name); setEditing({...t}); }}><Settings size={15}/>Editar tÃ©cnico</button>
    </article>)}</div>
    {editing && <div className="modal-backdrop" onMouseDown={() => setEditing(null)}><div className="modal technician-modal" onMouseDown={e => e.stopPropagation()}>
      <div className="modal-head"><div><p className="eyebrow">EQUIPO TÃ‰CNICO</p><h2>Editar tÃ©cnico</h2></div><button onClick={() => setEditing(null)} aria-label="Cerrar"><X/></button></div>
      <div className="form-grid">
        ...
        <label>TelÃ©fono<input value={editing.phone} onChange={e => setEditing({...editing, phone:e.target.value})}/></label>
        ...
        <label className="wide">Especialidad<input value={editing.specialty} onChange={e => setEditing({...editing, specialty:e.target.value})} placeholder="Ej. PlomerÃ­a Â· Boilers"/></label>
    ...
    {creating && <div className="modal-backdrop user-modal-backdrop" onMouseDown={() => !creatingUser && setCreating(null)}>
      <form id="new-technician-dialog" role="dialog" aria-modal="true" aria-labelledby="new-technician-title" className="modal user-modal" onSubmit={createTechnician} onMouseDown={event => event.stopPropagation()}>
        <div className="modal-head"><div><p className="eyebrow">NUEVO ACCESO TÃ‰CNICO</p><h2 id="new-technician-title">Crear tÃ©cnico</h2></div><button type="button" disabled={creatingUser} onClick={() => setCreating(null)} aria-label="Cerrar formulario"><X/></button></div>
        <div className="form-grid">
          ...
          <label>Correo electrÃ³nico<input required type="email" autoComplete="email" value={creating.email} onChange={event => setCreating({ ...creating, email: event.target.value })}/></label>
          <label>TelÃ©fono opcional<input inputMode="tel" autoComplete="tel" value={creating.phone} onChange={event => setCreating({ ...creating, phone: event.target.value })}/></label>
          <label className="wide">Especialidad<input value={creating.specialty} onChange={event => setCreating({ ...creating, specialty: event.target.value })} placeholder="Ej. PlomerÃ­a Â· Electricidad Â· Boilers"/></label>
          <label>Rol<input readOnly value="TÃ©cnico"/></label>
          ...
          <label>ContraseÃ±a temporal<div className="password-field"><KeyRound/><input required type="password" autoComplete="new-password" minLength={8} value={creating.password} onChange={event => setCreating({ ...creating, password: event.target.value })} placeholder="MÃ­nimo 8 caracteres"/></div></label>
          <label>Confirmar contraseÃ±a<div className="password-field"><KeyRound/><input required type="password" autoComplete="new-password" minLength={8} value={creating.confirm_password} onChange={event => setCreating({ ...creating, confirm_password: event.target.value })} placeholder="Repite la contraseÃ±a"/></div></label>
          <p className="technician-server-note wide"><ShieldCheck/>La cuenta se crea de forma segura en Supabase Auth y se vincula automÃ¡ticamente con el catÃ¡logo de TÃ©cnicos.</p>
          <div className="modal-actions wide"><button type="button" className="secondary-button" disabled={creatingUser} onClick={() => setCreating(null)}>Cancelar</button><button type="submit" className="primary-button" disabled={creatingUser}>{creatingUser ? "Creando tÃ©cnicoâ€¦" : "Crear tÃ©cnico"}</button></div>
        </div>
      </form>
    </div>}
  </section>;
...
function UserAdministration({ notify, onTechniciansChanged }: { notify: (message: string) => void; onTechniciansChanged: () => Promise<void> }) {
  ...
  const request = async (url = "/api/admin/users", options?: RequestInit) => {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(result.error || "No se pudo completar la operaciÃ³n");
    return result;
  };
  const load = async () => {
  ...
  useEffect(() => {
    if (!supabase) {
      const timer = window.setTimeout(() => {
        setError("No fue posible abrir la conexiÃ³n segura. Reintenta en unos segundos.");
        setLoading(false);
      }, 0);
      return () => window.clearTimeout(timer);
    }
    let active = true;
    withTimeout(supabase.auth.getSession(), "Supabase tardÃ³ demasiado en recuperar la sesiÃ³n.")
      .then(({ data, error: sessionError }) => {
        if (sessionError) throw sessionError;
        if (!data.session) throw new Error("Tu sesiÃ³n terminÃ³. Cierra sesiÃ³n e ingresa nuevamente.");
  ...
  const save = async () => {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(editing.email.trim())) return setFormError("Escribe un correo electrÃ³nico vÃ¡lido.");
    if (!editing.id && editing.password.length < 8) return setFormError("La contraseÃ±a temporal debe tener al menos 8 caracteres.");
    if (!editing.id && editing.password !== editing.confirm_password) return setFormError("Las contraseÃ±as no coinciden.");
    if (editing.id && editing.password && editing.password !== editing.confirm_password) return setFormError("Las contraseÃ±as no coinciden.");
    setSaving(true);
    try {
      ...
      window.dispatchEvent(new Event("izza:technicians-changed"));
      notify(editing.id ? "Usuario actualizado en Supabase" : "Usuario creado y listo para iniciar sesiÃ³n");
  ...
  const remove = async (user: SystemUser) => {
    if (user.protected || !window.confirm(`Â¿Eliminar el acceso de ${user.full_name}? Esta acciÃ³n no se puede deshacer.`)) return;
    try {
  ...
  const roleName = (role: AppRole | null) => role === "admin" ? "Administrador" : role === "reception" ? "RecepciÃ³n" : role === "technician" ? "TÃ©cnico" : "Sin perfil";
  ...
  return <section className="module-card user-admin">
    ...
    {loading ? <div className="user-empty">Consultando accesosâ€¦</div> : users.length === 0 ? <div className="user-empty">No hay usuarios registrados.</div> :
      ...
      {users.map(user => <div className="user-row" key={user.id}>
        <span><strong>{user.full_name}</strong><small>{user.email}</small><small>{user.phone || "Sin telÃ©fono"}</small></span>
        <span><b className={`role-pill role-${user.role || "unconfigured"}`}>{roleName(user.role)}</b>{!user.profile_configured && <small>Configura este acceso</small>}{user.role === "technician" && <small>{user.technician ? "Ficha vinculada" : "VinculaciÃ³n pendiente"}</small>}</span>
        <span><StatusBadge status={user.active ? "Activo" : "Inactivo"}/></span>
        <span><small>{user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleString("es-MX") : "AÃºn no ingresa"}</small></span>
    ...
    {editing && <div className="modal-backdrop user-modal-backdrop" onMouseDown={() => !saving && setEditing(null)}><form id="new-user-dialog" role="dialog" aria-modal="true" aria-labelledby="user-dialog-title" className="modal user-modal" onSubmit={event => { event.preventDefault(); void save(); }} onMouseDown={event => event.stopPropagation()}>
      ...
      {editing.protected && <div className="protected-note"><ShieldCheck/>Esta es la cuenta propietaria. Siempre conservarÃ¡ el rol Administrador y permanecerÃ¡ activa.</div>}
      ...
      <div className="form-grid">
        ...
        <label>Correo electrÃ³nico<input required type="email" autoComplete="email" value={editing.email} onChange={e => setEditing({...editing, email:e.target.value})}/></label>
        <label>TelÃ©fono opcional<input inputMode="tel" autoComplete="tel" value={editing.phone} onChange={e => setEditing({...editing, phone:e.target.value})}/></label>
        <label>Rol<select disabled={editing.protected} value={editing.role} onChange={e => setEditing({...editing, role:e.target.value as AppRole})}><option value="admin">Administrador</option><option value="reception">RecepciÃ³n</option><option value="technician">TÃ©cnico</option></select></label>
        ...
        {editing.role === "technician" && <label className="wide">VinculaciÃ³n con catÃ¡logo de tÃ©cnicos<select value={editing.technician_id} onChange={e => setEditing({...editing, technician_id:e.target.value})}><option value="">Crear una ficha tÃ©cnica nueva</option>{technicianCatalog.map(technician => <option key={technician.id} value={technician.id}>{technician.name}{technician.phone ? ` Â· ${technician.phone}` : ""}</option>)}</select><small>Selecciona una ficha real existente sin usuario, o crea una nueva automÃ¡ticamente.</small></label>}
        <label>ContraseÃ±a temporal {editing.id && "(opcional)"}<div className="password-field"><KeyRound/><input required={!editing.id} type="password" autoComplete="new-password" minLength={editing.id ? undefined : 8} value={editing.password} onChange={e => setEditing({...editing, password:e.target.value})} placeholder={editing.id ? "Conservar contraseÃ±a actual" : "MÃ­nimo 8 caracteres"}/></div></label>
        <label>Confirmar contraseÃ±a<div className="password-field"><KeyRound/><input required={!editing.id || Boolean(editing.password)} type="password" autoComplete="new-password" value={editing.confirm_password} onChange={e => setEditing({...editing, confirm_password:e.target.value})} placeholder="Repite la contraseÃ±a"/></div></label>
